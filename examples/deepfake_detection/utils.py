...
class ImagePreprocessor:
    ...
    def _process_video(self, video_input: torch.Tensor, fps: float) -> torch.Tensor:
        ...
        if self.window_len == -1:  # use the entire video
            ...
            usable_frames = (
                num_frames // fps
            ) * fps  # drop the last few frames if not enough for a window

            # process video in windows
            windows = [
                video_input[i : i + frames_per_window]
                for i in range(0, usable_frames, step_frames)
            ]
...
class AudioPreprocessor:
    ...
    def _process_audio(self, audio_input: torch.Tensor, sr: int) -> torch.Tensor:
        ...
        if self.window_len == -1:  # use the entire audio
            ...
            ) * self.sample_rate  # drop the last few samples if not enough for a window

            windows = [
                audio_input[i : i + window_samples]
                for i in range(0, usable_samples, step_samples)
            ]
