...
def main():
    ...
    ap.add_argument("--ratio", type=float, default=0.9,
                    help="fraction of subjects that go to train")
    ...
    df = pd.DataFrame(train_rows + dev_rows,
                      columns=["sample_id", "id_target", "n_win"])

    mapping = decide_partition(df["id_target"].unique(),
                               ratio=args.ratio, seed=args.seed)

    # open new files
    dst = {s: h5py.File(new_root / f"{s}.h5", "w")
           for s in ("train", "dev")}
